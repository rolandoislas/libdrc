...
int get_interface_of_ipv4(const std::string &addr, std::string &out)
{
	...
	while(p)
	{
		if(p->ifa_addr != NULL)
		{
			...
			{
				rv = getnameinfo(p->ifa_addr, sizeof(struct sockaddr_in),
					ip_buffer, sizeof(ip_buffer), NULL, 0, NI_NUMERICHOST);
				if(rv == 0)
				{
					if(addr == ip_buffer) break;
				}
			}
		}
		p = p->ifa_next;
...
int get_tsf(u64 &tsf) {
  static int fd = -1;
  if (fd == -1) {
    std::string drc_iface;
    if(get_interface_of_ipv4("192.168.1.10", drc_iface) == 0)
    {
      std::string tsf_path = std::string("/sys/class/net/") + drc_iface + "/tsf";
...
int main()
{
	...
	if(rv != 0)
	{
		std::printf("could not find local ip 192.168.1.10; is this machine connected to drc?\n");
	}
	else
	{
		rv = drc::get_tsf(tsf);
